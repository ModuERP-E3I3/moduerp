...
public interface EmpMgtService {
    ...
    // 직원 등록
    void createEmployee(Employee empMgtDTO);
    ...
    List<Employee> getEmployeesByBizNumber(String bizNumber);

    // 부서와 직급 정보를 함께 가져오기
    List<Employee> getEmpNameDepart(String bizNumber);

    // UUID로 직원 상세 정보 가져오기
    Employee getEmployeeDetailByUUID(String uuid);

    // 직원 정보 수정
    void updateEmployee(Employee empMgtDTO);

    // UUID로 직원 삭제
    void deleteEmployeeByUUID(String uuid);

    // 필터: 부서명으로 직원 목록 가져오기
    List<Employee> getEmployeesByDepartmentName(String bizNumber, String departmentName);

    // 필터로 직원 목록 가져오기
    List<Employee> getEmployeesByFilter(String bizNumber, String option, String filterText);

    // 직급 ID로 직원 목록 가져오기
    List<Employee> getEmployeesByJobId(String bizNumber, String jobId);

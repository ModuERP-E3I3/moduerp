		...
		<table>
			<thead id="carTable">
				...
				<tr>
					<c:forEach var="car" items="${carList}">
						<th
							onclick="window.location.href='getCarDetail.do?carId=${ car.carId }' ">차량
							이미지</th>
					</c:forEach>
				</tr>
			...
			<tbody>
				<tr>
					...
					<c:forEach var="car" items="${carList}">
						<td
							onclick="window.location.href='getCarDetail.do?carId=${ car.carId }' "><img
							alt="${car.carModel}"
							src="${pageContext.request.contextPath}/${car.imagePath}"
							width="25%"></td>
					</c:forEach>
				</tr>
			</tbody>
			...
			<thead>
				<tr>
					<c:forEach var="car" items="${carList}">
						<th
							onclick="window.location.href='getCarDetail.do?carId=${ car.carId }' ">차종</th>
					</c:forEach>
				</tr>
			</thead>
			<tbody>
				<tr>
					...
					<c:forEach var="car" items="${carList}">
						<td
							onclick="window.location.href='getCarDetail.do?carId=${ car.carId }' ">${car.carModel}</td>
					</c:forEach>
				</tr>
			</tbody>
			...
			<tbody>
				<tr>
					...
					<c:forEach var="car" items="${carList}">
						<td
							onclick="window.location.href='getCarDetail.do?carId=${ car.carId }' ">${car.carNum}</td>
					</c:forEach>
				</tr>
			</tbody>
			...
			<thead>
				<tr>
					<c:forEach var="car" items="${carList}">
						<th
							onclick="window.location.href='getCarDetail.do?carId=${ car.carId }' ">소유
							형태</th>
					</c:forEach>
				</tr>
			</thead>
			<tbody>
				<tr>
					...
					<c:forEach var="car" items="${carList}">
						<td
							onclick="window.location.href='getCarDetail.do?carId=${ car.carId }' ">${car.ownershipStatus}</td>
					</c:forEach>
				</tr>
			</tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const carCount = ${fn:length(carList)}; // 데이터 개수 가져오기
    console.log(carCount);
    
    // 전체 테이블에 클래스 적용
    if (carCount === 1) {
        carTable.closest('table').classList.add('table-one'); // 1개일 경우 클래스 추가
    } else if (carCount === 2) {
        carTable.closest('table').classList.add('table-two'); // 2개일 경우 클래스 추가
    }
});
    </script>
